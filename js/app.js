...
var APP_CHANGELOG = [
  ...
  "V7.2.0 TEST — Nouveau Mode Atelier plein écran : fiche de fabrication consolidée sans modifier le fonctionnement existant.",
  "V7.1.0 PROD — Nouveau module Campagnes e-mail : filtre des devis envoyés avant une date, sélection manuelle, personnalisation et historique.",
  ...
  "V5.0.1 — Correctif d’affichage et de conservation des photos d’inspiration du portail.",
  "V4.2.1 PROD — Correctif mariage : les coordonnées enregistrées sont propagées à la fiche cliente et au devis lié sans reconstruire les lignes tarifées.",
  "V4.2.1 PROD — Protection anti-devis à 0 € : conservation des prix lors des synchronisations et tentative de restauration depuis une facture ou commande liée avant envoi.",
  "V4.2.0 PROD — Ateliers multi-activités : plusieurs choix à une même date, capacité et tarif par choix, réservations et stock ventilés par activité.",
  "V4.2.0 PROD — Correction du compteur réservé/capacité et du badge COMPLET pour les ateliers privés.",
  "V4.1.3 — Affichage des participantes Squarespace dans tous les types d’ateliers et réparation automatique des liens manquants.",
  "V4.1.2 — Modification des encaissements Squarespace depuis l’historique et recalcul automatique du bon atelier.",
  "V4.1.1 — Tous les ateliers non annulés visibles, nombre de places par commande, calcul acompte/total et décompte exact des places.",
  "V4.1.0 — Nouveaux ateliers visibles dans les réservations site, séparation paiements atelier / ventes indépendantes, suivi des places et suppression du double affichage du CA.",
...
function portalDateISO(v){
  if(!v) return "";
  if(typeof v==="string") return v;
  try{ if(v.toDate) return v.toDate().toISOString(); }catch(e){}
...
function portalProjectToDemande(doc){
  var x=doc.data()||{};
  return {id:doc.id,ownerUid:x.ownerUid||doc.id,prenom:x.prenom||"",nom:x.nom||"",email:x.email||"",tel:x.tel||"",dateMariage:x.dateMariage||"",adressePostale:x.adressePostale||"",codePostal:x.codePostal||"",ville:x.ville||"",lieu:x.lieu||"",invites:x.invites||"",style:x.style||"",budget:x.budget||"",prestations:Array.isArray(x.prestations)?x.prestations:[],prestationQuantites:(x.prestationQuantites&&typeof x.prestationQuantites==="object")?x.prestationQuantites:{},rubansPersonnalises:(x.rubansPersonnalises&&typeof x.rubansPersonnalises==="object")?x.rubansPersonnalises:{},autrePrestation:x.autrePrestation||"",couleurs:x.couleurs||"",fleursAimees:x.fleursAimees||"",fleursPremium:Array.isArray(x.fleursPremium)?x.fleursPremium:[],fleursPremiumQuantites:(x.fleursPremiumQuantites&&typeof x.fleursPremiumQuantites==="object")?x.fleursPremiumQuantites:{},fleursPremiumAutre:x.fleursPremiumAutre||"",fleursRefusees:x.fleursRefusees||"",canal:x.canal||"Portail sécurisé",description:x.description||"",souhaiteRdvTelephonique:x.souhaiteRdvTelephonique||"",rdvDateSouhaitee:x.rdvDateSouhaitee||"",rdvHeureSouhaitee:x.rdvHeureSouhaitee||"",photos:Array.isArray(x.photos)?x.photos:[],statut:x.statutAdmin||"nouvelle",createdAt:portalDateISO(x.createdAt)||new Date().toISOString(),updatedAt:portalDateISO(x.updatedAt),clientModificationHistory:Array.isArray(x.clientModificationHistory)?x.clientModificationHistory:[],clientModificationPending:!!x.clientModificationPending,clientModificationCount:Number(x.clientModificationCount)||0,lastClientModificationAt:portalDateISO(x.lastClientModificationAt)||x.lastClientModificationAt||"",lastClientChanges:Array.isArray(x.lastClientChanges)?x.lastClientChanges:[],securePortal:true};
}
...
function publishSecureClientSpaces(){
  // V8 TIIME : l’espace mariage reste un espace de suivi uniquement.
  // On synchronise la fiche/planning, mais plus aucun devis ni facture n’est déposé dans portalDocuments.
...
function fieldLabelSafe(path){
  var labels={nom:"Nom",email:"Email",tel:"Téléphone",dateMariage:"Date du mariage",dateLivraison:"Date de livraison",adressePostale:"Adresse postale",codePostal:"Code postal",lieu:"Lieu",ville:"Ville",memo:"Mémo",synthese:"Synthèse",besoins:"Ses besoins",notes:"Notes",notesInternes:"Notes internes",notesAtelier:"Notes atelier",medias:"Inspirations",media:"Inspirations",inspirations:"Inspirations",articles:"Créations / articles",statut:"Statut"};
...
function viewClientsModule(){
  ...
  var html=moduleHeader("Clients","Fiches clientes, mariages, ateliers, ventes et commandes regroupés au même endroit.")+
    subNav([["clients","Fiches clientes"],["demandesMariage","Demandes mariage"],["mariages","Mariages"],["ateliers","Ateliers"],["commandes","Suivi des Commandes"],["ventesSite","Paiements et ventes du site"],["encaissements","Encaissements manuels"]],sub,"mod-clients-");
  if(sub==="demandesMariage") return html+viewDemandesMariage();
...
function renderModal(){
  ...
  if(ui.preview){
    if(ui.preview.kind==="devis" && recoverDevisLinesIfZero(ui.preview.doc)){
      saveCache();
      toast("Montant du devis restauré depuis un document lié.");
    }
...
function anneesDispo(){
  ...
  (state.ventesSite||[]).forEach(function(v){ if(v.date) s[v.date.slice(0,4)]=1; });
...
function viewVersionDashboard(){
  var env = APP_VERSION.indexOf("PROD")>=0 ? "PROD" : "DEVELOPPEMENT";
  ...
  return '<div class="card" style="background:'+bg+';border-color:'+border+';margin-bottom:12px;padding:10px 14px;">'+
    '<div class="flexb" style="gap:8px;">'+
      '<div><b style="color:'+(env==="PROD"?'var(--green)':'var(--bordeaux)')+';">'+(env==="PROD"?'✅':'🧪')+' Version : '+esc(APP_VERSION)+'</b><div class="muted" style="font-size:12px;margin-top:2px;">☁ Synchronisation active · '+esc(APP_VERSION_NOTE)+'</div></div>'+ 
...
function monthSales(year, mi){
  ...
  (state.ventesSite||[]).forEach(function(s){
    if(s.date && s.date.slice(0,7)===pref){
      ...
      items.push({date:s.date, label:(s.commande?("Squarespace "+s.commande+" · "):"")+(s.produit||"Opération site"), client:s.client||"", montant:sp.total, biens:sp.biens, services:sp.services, type:siteSaleNatureLabel(s)});
    }
  });
...
function isSiteSaleAtelierPayment(s){
  if(!s) return false;
  if(s.nature==="atelier_payment") return true;
  if(s.nature==="independent_sale") return false;
  return !!(s.atelierId && String(s.activite||"").toLowerCase().indexOf("atelier")>=0);
}
function normalizeSiteSalesData(){
  state.ventesSite=(state.ventesSite||[]).map(function(s){
    s=s||{};
...
function setSiteDraftAtelier(s,atelierId,resetAmount){
  if(!s) return;
  var eligible=siteEligibleAteliers();
  var a=eligible.find(function(x){return x.id===atelierId;})||eligible[0]||null;
  s.atelierId=a?a.id:"";
...
function siteSalesSummary(list){
  var out={atelierPayments:0,independentSales:0,totalReceived:0,countAtelier:0,countIndependent:0};
  (list||[]).forEach(function(s){
    var amount=Number(s.montant)||0;
    out.totalReceived+=amount;
    if(isSiteSaleAtelierPayment(s)){ out.atelierPayments+=amount; out.countAtelier++; }
    else { out.independentSales+=amount; out.countIndependent++; }
  });
  out.atelierPayments=r2(out.atelierPayments);
  out.independentSales=r2(out.independentSales);
  out.totalReceived=r2(out.totalReceived);
  return out;
}

function newSiteSaleDraft(){
  ui.siteSaleEditingId=null;
  var firstAtelier=siteEligibleAteliers()[0]||null;
  ui.siteSaleDraft={
    id:uid(),
    date:todayISO(),
  ...
  setSiteDraftAtelier(ui.siteSaleDraft,ui.siteSaleDraft.atelierId,true);
}

function editSiteSale(id){
  var original=(state.ventesSite||[]).find(function(s){return s.id===id;});
  if(!original){ toast("Encaissement introuvable."); return; }
  var s=Object.assign({},original);
  s.id=original.id;
  s.clientMode=(s.clientId&&(state.clients||[]).some(function(c){return c.id===s.clientId;}))?"existant":"nouveau";
  s.activite=isSiteSaleAtelierPayment(s)?"Atelier":(s.activite||"Autre");
  if(s.activite==="Atelier"){
    s.nature="atelier_payment";
    s.caContractuelInclus=true;
    s.nbPlaces=sitePlacesCount(s.nbPlaces||s.nombrePlaces||1);
    var storedTotal=Number(s.atelierPrixTotal)||0;
    var unit=Number(s.atelierPrixUnitaire)||Number(s.prixUnitaire)||0;
    ...
    s.atelierPaiementType=s.atelierPaiementType==="acompte30"?"acompte30":"total";
    s.prestation=s.prestation||s.produit||"Place atelier";
    if(linkedAtelier) s.atelierLabel=siteAtelierDisplayLabel(linkedAtelier);
  }else{
    s.nature="independent_sale";
    s.caContractuelInclus=false;
  }
  ui.siteSaleDraft=s;
  ui.siteSaleEditingId=id;
  ui.confirmDelete=null;
  ui.tab="clientsModule";
  ui.clientsSub="ventesSite";
  render();
  setTimeout(function(){ window.scrollTo(0,0); },0);
}
function safeSiteUnitPrice(total,places){
  places=sitePlacesCount(places);
  return r2((Number(total)||0)/places);
}

function captureSiteSaleDraft(){
  var s=ui.siteSaleDraft; if(!s) return;
  s.date=val("siteDate")||s.date||todayISO();
  s.commande=val("siteCommande");
  s.activite=val("siteActivite")||s.activite||"Autre";
  ...
  s.produit=val("siteProduit")||s.produit||"";
...
function viewSiteSaleForm(){
  ...
  var editing=!!ui.siteSaleEditingId;
  var clientPart='';
  if(state.clients.length){
  ...
  var isAtelier=s.activite==="Atelier";
  return '<div class="card" style="border-color:var(--gold-s);"><div class="flexb" style="margin-bottom:10px;"><h3 style="margin:0;">'+(editing?(isAtelier?'Modifier le paiement atelier':'Modifier la vente indépendante'):(isAtelier?'Nouveau paiement atelier via le site':'Nouvelle vente site indépendante'))+'</h3><button class="btn small ghost" data-action="site-cancel">Annuler</button></div>'+
    (editing?'<div class="summary" style="margin-bottom:12px;background:var(--gold-s);"><div class="totrow"><span>Correction d’un encaissement existant</span><b>Le précédent rattachement sera remplacé</b></div></div>':'')+
    '<p class="muted" style="margin-top:0;">'+(isAtelier?'Le paiement sera rattaché à l’atelier et diminuera son reste à encaisser. Il ne sera pas ajouté une seconde fois au CA prévu de l’atelier.':'Cette vente est indépendante d’un atelier et constitue un nouveau chiffre d’affaires encaissé.')+'</p>'+
    ...
    clientPart+
    '<div class="inline"><div><label class="field"><span>Type d’opération</span><select id="siteActivite" data-action="site-activite-change">'+siteActiviteOptions(s.activite)+'</select></label></div>'+
    '<div><label class="field"><span>Montant réellement encaissé sur Squarespace (€)</span><input id="siteMontant" type="number" min="0" step="0.01" value="'+esc(s.montant||"")+'"><div class="hint">Ce montant reste compté dans la trésorerie et l’URSSAF, car il a réellement été encaissé.</div></label></div></div>'+
    atelierPart+
    ...
    '<label class="field"><span>Notes</span><textarea id="siteNotes" placeholder="Informations utiles…">'+esc(s.notes||"")+'</textarea></label>'+
    '<div class="summary"><div class="totrow"><span>Nature comptable dans MyBusiness</span><b>'+(isAtelier?'Paiement d’un atelier existant':'Vente indépendante')+'</b></div><div class="totrow"><span>Moyen de paiement</span><b>Site internet</b></div><div class="totrow"><span>Statut</span><b>Payé</b></div></div>'+
    '<div class="row-actions"><button class="btn primary" data-action="site-save">'+(editing?'Enregistrer les corrections':(isAtelier?'Enregistrer le paiement atelier':'Enregistrer la vente indépendante'))+'</button></div></div>';
}

function viewVentesSite(){
  var list=(state.ventesSite||[]).slice().sort(function(a,b){return (b.date||"").localeCompare(a.date||"");});
  var now=todayISO();
  var monthList=list.filter(function(s){return (s.date||"").slice(0,7)===now.slice(0,7);});
  var yearList=list.filter(function(s){return (s.date||"").slice(0,4)===now.slice(0,4);});
  var month=siteSalesSummary(monthList), year=siteSalesSummary(yearList);
  var html='<div class="flexb" style="margin-bottom:14px;"><h2 style="margin:0;">Paiements et ventes du site</h2><button class="btn primary" data-action="site-new">+ Nouvelle opération site</button></div>'+
    '<p class="muted" style="margin-top:-6px;">Les réservations d’atelier sont traitées comme des <b>paiements rattachés</b>. Les box, produits et autres commandes restent des <b>ventes indépendantes</b>. Les deux alimentent la trésorerie réelle, mais seuls les produits indépendants créent un CA contractuel supplémentaire.</p>'+
    '<div class="grid-stats">'+stat("Paiements ateliers ce mois",euro(month.atelierPayments),false,'var(--green-s)')+stat("Ventes indépendantes ce mois",euro(month.independentSales),false,'var(--blush-s)')+stat("Total encaissé via le site",euro(month.totalReceived),true)+stat("Ventes indépendantes cette année",euro(year.independentSales),false)+'</div>';
  if(ui.siteSaleDraft) html+=viewSiteSaleForm();
  if(!list.length) return html+'<div class="card"><p class="muted" style="margin:0;">Aucun paiement ou vente site enregistré.</p></div>';
  html+='<div class="card"><h3 style="margin:0 0 10px;">Historique Squarespace</h3>';
  list.forEach(function(s){
    var del=ui.confirmDelete==="site:"+s.id;
    var linked=isSiteSaleAtelierPayment(s);
    html+='<div class="checkrow" style="align-items:flex-start;"><div style="flex:1;">'+
      '<div><b style="color:var(--bordeaux);">'+esc(s.produit||"Opération site")+'</b> <span class="achat-cat">'+esc(siteSaleNatureLabel(s))+'</span></div>'+
      ...
      '</div><div style="text-align:right;"><b style="color:var(--bordeaux);">'+euro(s.montant||0)+' encaissé</b><div class="row-actions" style="margin-top:6px;justify-content:flex-end;"><button class="btn small ghost" data-action="site-edit-'+s.id+'">Modifier</button><button class="btn small danger" data-action="site-del-'+s.id+'">'+(del?'Confirmer':'Supprimer')+'</button></div></div></div>';
  });
  html+='</div>';
  return html;
}
...
function saveSiteSale(){
  ...
  var s=ui.siteSaleDraft; if(!s) return;
  var editingId=ui.siteSaleEditingId||"";
  var existingIndex=editingId?(state.ventesSite||[]).findIndex(function(x){return x.id===editingId;}):-1;
  var oldSale=existingIndex>=0?state.ventesSite[existingIndex]:null;
  if(!(s.client||"").trim()){ toast("Indique la cliente."); return; }
  if(s.commande && (state.ventesSite||[]).some(function(x){return x.commande===s.commande&&x.id!==editingId;})){
    toast("Cette commande Squarespace est déjà enregistrée."); return;
  }
  if(s.activite==="Atelier"){
  ...
  if(!(s.produit||"").trim()) s.produit=(s.activite==="Atelier"?(s.prestation||"Réservation atelier"):"Vente indépendante du site");
  s.updatedAt=new Date().toISOString();

  state.ventesSite=state.ventesSite||[];
  if(oldSale){
    s.id=oldSale.id;
    s.createdAt=oldSale.createdAt||s.createdAt||new Date().toISOString();
    removeSiteSaleFromAtelier(oldSale);
    syncSiteSaleToAtelier(s);
    state.ventesSite[existingIndex]=Object.assign({},s);
  }else{
    syncSiteSaleToAtelier(s);
  ...
  ui.siteSaleEditingId=null;
  saveCache(); render();
  toast(oldSale?"Encaissement corrigé : le décompte du bon atelier a été recalculé.":(isSiteSaleAtelierPayment(s)?"Paiement rattaché à l’atelier sans double comptage du CA.":"Vente site indépendante enregistrée."));
}

function viewSiteSalesPreview(){
  var all=(state.ventesSite||[]).slice().sort(function(a,b){return (b.date||"").localeCompare(a.date||"");});
  var list=all.slice(0,3);
  var month=siteSalesSummary(all.filter(function(s){return (s.date||"").slice(0,7)===todayISO().slice(0,7);}));
  var html='<div class="card"><div class="flexb"><h3 style="margin:0;">🌐 Squarespace</h3><button class="btn small primary" data-action="nav-ventesSite">Ouvrir</button></div>'+
    '<p class="muted" style="margin:8px 0 10px;">Encaissé ce mois : <b style="color:var(--bordeaux);">'+euro(month.totalReceived)+'</b> · dont '+euro(month.atelierPayments)+' de paiements ateliers.</p>';
  if(!list.length) return html+'<p class="muted" style="margin:0;">Aucune opération site enregistrée.</p></div>';
  list.forEach(function(s){ html+='<div class="cal-listitem"><b style="color:var(--bordeaux);">'+esc(s.produit||"Opération site")+'</b><div class="muted" style="font-size:12px;">'+frDate(s.date)+' · '+esc(siteSaleNatureLabel(s))+' · '+esc(s.client||"")+' · '+euro(s.montant||0)+'</div></div>'; });
  return html+'</div>';
}
...
function encaissementsTresorerie(){
  ...
  (state.ventesSite||[]).forEach(function(s){
    arr.push({
      ...
      libelle:(s.commande?("Squarespace "+s.commande):siteSaleNatureLabel(s))+" · "+(s.produit||""),
      client:s.client||"",
      ...
      facture:null,
      siteSale:s,
      nature:isSiteSaleAtelierPayment(s)?"atelier_payment":"independent_sale"
    });
  });
...
function todaySmartData(){
  ...
  var siteM=siteSalesSummary((state.ventesSite||[]).filter(function(s){return (s.date||"").slice(0,7)===today.slice(0,7);})).totalReceived;
...
function viewSmartDashboard(){
  ...
  return '<div class="card" style="border-color:var(--bordeaux);"><h3 style="margin:0 0 10px;">Aujourd’hui</h3>'+
    ...
    '<div class="checkrow" data-action="nav-ventesSite" style="cursor:pointer;"><div style="flex:1;">🌐 Encaissements Squarespace du mois</div><b>'+euro(d.siteM)+'</b></div>'+
...
function clientHistoriqueComplet(c){
  ...
  (state.ventesSite||[]).forEach(function(s){
    if(s.client && clientNameMatches(c.nom,s.client)){
      items.push({date:s.date,type:siteSaleNatureLabel(s),label:(s.commande?("Squarespace "+s.commande+" · "):"")+ (s.produit||"Opération site"),montant:Number(s.montant)||0,paid:true,source:"site",id:s.id,paiement:"Site internet",activite:s.activite||""});
    }
  });
  (state.ateliers||[]).forEach(function(a){
    (a.participants||[]).forEach(function(p){
      if(p.nom && clientNameMatches(c.nom,p.nom) && !p.siteSaleId){
...
function viewAtelierRecipeManager(){
  ...
  var html='<div class="card" id="atelierRecipeManager"><div class="flexb" style="align-items:flex-start;"><div><h3 style="margin:0;">🧰 Recettes matériel des ateliers</h3><p class="muted" style="margin:5px 0 0;">Choisis un atelier puis définis le matériel consommé <b>pour une personne</b>. Le stock sera retiré au moment de « Préparer le matériel ».</p></div><span class="badge" style="background:var(--green-s);color:var(--green);">Décompte actif</span></div>'+ 
  ...
  html+='<div class="row-actions"><button class="btn soft" type="button" data-action="stock-recipe-add">+ Ajouter du matériel</button><button class="btn primary" type="button" data-action="stock-recipe-save">Enregistrer la recette</button><button class="btn ghost" type="button" data-action="stock-recipe-reset">Rétablir la recette initiale</button></div>'+ 
    '<div class="summary" style="margin:12px 0 0;"><b>Créer un atelier saisonnier</b><div class="muted">Va dans l’onglet « Bibliothèque des ateliers », crée ton thème Noël, puis reviens ici pour lui associer les articles et quantités nécessaires.</div></div></div>';
  return html;
}
function atelierStockSnapshotFromRecipe(a){
  return atelierRecipe(a).map(function(r){
    return {stockItemId:r.stockItemId||((r.item&&r.item.id)||""),label:r.label||((r.item&&r.item.nom)||"Matériel"),qty:r2(Number(r.qty)||0),unit:(r.item&&r.item.unite)||"pièce"};
  }).filter(function(x){return x.stockItemId&&x.qty>0;});
}
function atelierStockMap(lines){
  var out={}; (lines||[]).forEach(function(x){if(x&&x.stockItemId) out[x.stockItemId]=x;}); return out;
}
function atelierStockShortages(deltas){
  var miss=[];
  Object.keys(deltas||{}).forEach(function(id){
    var d=Number(deltas[id])||0; if(d<=0) return;
    var it=(state.stockItems||[]).find(function(x){return x.id===id;});
    if(!it) miss.push({label:"Article supprimé ou introuvable",available:0,needed:r2(d),unit:""});
    else if((Number(it.quantite)||0)+0.000001<d) miss.push({label:it.nom||"Article",available:r2(Number(it.quantite)||0),needed:r2(d),unit:it.unite||"pièce"});
  });
  return miss;
}
function atelierStockDelta(current,next){
  var a=atelierStockMap(current), b=atelierStockMap(next), ids={}, out={};
  Object.keys(a).forEach(function(k){ids[k]=true;}); Object.keys(b).forEach(function(k){ids[k]=true;});
  Object.keys(ids).forEach(function(id){out[id]=r2(((b[id]&&Number(b[id].qty))||0)-((a[id]&&Number(a[id].qty))||0));});
  return out;
}
function atelierStockSummary(lines,title){
  var h=(title?title+"\n\n":"");
  (lines||[]).forEach(function(x){h+="• "+(x.label||"Matériel")+" : "+r2(x.qty)+" "+(x.unit||"")+"\n";});
  return h;
}
function atelierStockRecord(item,from,to,context){
  item.quantite=r2(to); item.updatedAt=new Date().toISOString();
...
function atelierApplyStockDelta(a,deltas,context,allowNegative){
  ...
  Object.keys(deltas||{}).forEach(function(id){
    ...
    var from=Number(it.quantite)||0, to=r2(from-d);
    atelierStockRecord(it,from,to,context||("Atelier "+(a.theme||a.type||"")));
  });
  return {ok:true};
}
...
function atelierSitePaymentTotals(a){
  var paye=0, solde=0, saleIds={};
  (state.ventesSite||[]).forEach(function(s){
    if(!isSiteSaleAtelierPayment(s) || s.atelierId!==a.id) return;
    if(s.id && saleIds[s.id]) return;
    if(s.id) saleIds[s.id]=true;
    var paid=Number(s.montant)||0;
    var contract=Number(s.atelierPrixTotal)||paid;
    paye+=paid;
    solde+=Math.max(0,contract-paid);
  });
  ((a&&a.participants)||[]).forEach(function(p){
    var isSite=p.source==="site"||p.payeSite||String(p.facturation||"").indexOf("site")===0;
    if(!isSite || (p.siteSaleId&&saleIds[p.siteSaleId])) return;
    paye+=Number(p.payeSiteMontant)||Number(p.montant)||0;
    solde+=Number(p.soldeSite)||0;
  });
...
function atelierPaymentRows(a){
  var rows=[];
  (state.ventesSite||[]).forEach(function(s){
    if(isSiteSaleAtelierPayment(s)&&s.atelierId===a.id){
      rows.push({date:s.date||"",client:s.client||"Participante",source:"Squarespace",reference:s.commande||"",montant:Number(s.montant)||0,places:sitePlacesCount(s.nbPlaces),paiementType:s.atelierPaiementType||"total",contract:Number(s.atelierPrixTotal)||0,siteSaleId:s.id});
    }
  });
  atelierFacturesLiees(a).forEach(function(f){
    if(f.statut==="payee") rows.push({date:f.datePaiement||f.date||"",client:(f.client&&f.client.nom)||"Cliente",source:normalizePaymentMethod(f.paiementClient||"Facture payée"),reference:f.numero||"",montant:Number(f.montant)||0});
  });
  rows.sort(function(x,y){return (y.date||"").localeCompare(x.date||"");});
  return rows;
}
function viewAtelierPaymentSummary(a){
  var t=atelierTotals(a), cap=atelierCapacityInfo(a), rows=atelierPaymentRows(a);
  var reste=r2(Math.max(0,(Number(t.total)||0)-(Number(t.paye)||0)));
  var html='<div class="card" style="border-color:var(--gold-s);background:#fffaf5;"><div class="flexb"><div><h3 style="margin:0;">💳 Places et paiements</h3><p class="muted" style="margin:4px 0 0;">Le CA prévu de l’atelier reste unique. Chaque réservation payée réduit seulement le reste à encaisser.</p></div><span class="badge" style="background:var(--green-s);color:var(--green);">calcul automatique</span></div>'+
    '<div class="grid-stats" style="margin-top:12px;">'+
      stat("CA prévu atelier",euro(t.total),true)+
      stat("Montant encaissé",euro(t.paye),false,'var(--green-s)')+
      stat("Reste à encaisser",euro(reste),false,'var(--blush-s)')+
      stat("Places réservées",cap.reserved+(cap.planned?" / "+cap.planned:""),false)+
    '</div>'+
    '<div class="summary" style="margin-bottom:12px;"><div class="totrow"><span>Places encore disponibles</span><b>'+cap.remaining+'</b></div>'+(cap.overbooked?'<div class="totrow" style="color:var(--red);"><span>Dépassement de capacité</span><b>+'+cap.overbooked+'</b></div>':'')+'<div class="totrow"><span>Paiements Squarespace inclus dans l’encaissé</span><b>'+euro(t.sitePaye)+'</b></div></div>'+
...
function atelierTotals(a){
  ...
  var siteTotals=atelierSitePaymentTotals(a);
  var sitePaye=siteTotals.paye;
  var siteSolde=siteTotals.solde;
...
function viewStockEditModal(item){
  ...
  if(hist.length){
    ...
    hist.forEach(function(entry){
      h+='<div class="cal-listitem"><b>'+esc(stockDateTime(entry.date))+'</b>'+(entry.context?'<div style="font-size:12px;font-weight:700;">'+esc(entry.context)+'</div>':'')+'<div class="muted" style="font-size:12px;">'+(entry.changes||[]).map(function(c){return esc(stockHistoryLabel(c.field))+': '+esc(c.from==null?'':c.from)+' → '+esc(c.to==null?'':c.to);}).join('<br>')+'</div></div>';
    });
    h+='</div>';
  }
  h+='</div></div></div>';
...
function viewStock(){
  ...
  var sub=ui.stockSub||"articles";
  var html='<div class="flexb" style="margin-bottom:14px;"><h2 style="margin:0;">Matériel</h2><div class="row-actions" style="margin-top:0;"><span class="badge" style="background:var(--green-s);color:var(--green);">✓ Décompte ateliers actif</span><span class="muted">Bibliothèque centrale des fleurs, fournitures et ateliers</span></div></div>'+
    '<div class="subnav" style="margin-bottom:14px;"><button class="'+(sub==="articles"?'active':'')+'" data-action="stock-sub-articles">📦 Articles et stock</button><button class="'+(sub==="ateliers"?'active':'')+'" data-action="stock-sub-ateliers">📚 Bibliothèque des ateliers</button><button class="'+(sub==="recettes"?'active':'')+'" data-action="stock-sub-recettes">🧰 Recettes matériel</button></div>';
  if(sub==="ateliers") return html+viewAtelierLibraryManager();
  if(sub==="recettes") return html+viewAtelierRecipeManager();
  html+='<div class="grid-stats">'+stat("Valeur du matériel",euro(total),true)+stat("Quantité totale",q,false)+stat("Références",state.stockItems.length,false)+stat("Alertes",low,false)+'</div>'+
...
function exportCSV(annee){
  ...
  (state.ventesSite||[]).forEach(function(s){
    if(s.date&&(annee==="all"||s.date.slice(0,4)===String(annee))){
      var sp=siteSaleSplit(s);
      rows.push({date:s.date, source:siteSaleNatureLabel(s), ref:s.commande||"", type:s.produit||s.activite||"Opération site", client:s.client||"", montant:sp.total, biens:sp.biens, services:sp.services, paiement:"Site internet"});
    }
  });
...
/* ===================== V5.0 — Portail demandes mariage ===================== */
var DEMANDE_STATUTS={nouvelle:"Nouvelle demande",a_etudier:"À étudier",a_contacter:"Cliente à contacter",contactee:"Contactée",rdv:"Rendez-vous programmé",a_completer:"Informations à compléter",validation:"En attente de validation",transformee:"Transformée en mariage",sans_suite:"Sans suite",indisponible:"Date indisponible"};
function normalizePortalMedia(){
  (state.demandesMariage||[]).forEach(function(d){ (d.photos||[]).forEach(function(p){ if(p&&typeof p==="object"&&!p.dataUrl&&p.data) p.dataUrl=p.data; }); });
...
function portailCreationCanonique(label){
  ...
  for(var i=0;i<PORTAIL_CREATIONS_STANDARD.length;i++){
    if(normName(PORTAIL_CREATIONS_STANDARD[i])===key) return PORTAIL_CREATIONS_STANDARD[i];
  }
  // Compatibilité avec les demandes créées dans les premières versions test.
  var aliases={
    "bouquet de mariee":"Bouquet de mariée taille M — 20 à 23 cm",
    "bouquet de mariee taille s 20 a 22 cm":"Bouquet de mariée taille S — 18 à 20 cm",
    "bouquet de mariee taille m 23 a 25 cm":"Bouquet de mariée taille M — 20 à 23 cm",
    "bouquet demoiselle d honneur":"Bouquet demoiselle d’honneur ou à lancer — diamètre 18 cm (recommandé adulte)",
    "bouquets demoiselles d honneur":"Bouquet demoiselle d’honneur ou à lancer — diamètre 18 cm (recommandé adulte)",
    "bouquet enfant demoiselle":"Bouquet demoiselle d’honneur ou à lancer — diamètre 15 cm (recommandé enfant)",
    "boutonnieres":"Boutonnière adulte sans mini-rose",
    "boutonniere s":"Boutonnière adulte sans mini-rose",
    "couronne de fleurs":"Couronne sur fil de fer adulte",
    "couronne fleurie":"Couronne sur fil de fer adulte",
    "centres de table":"Centre de table",
    "centre s de table":"Centre de table",
    "decoration de ceremonie":"Décoration cérémonie",
    "decor ceremonie arche":"Décoration cérémonie"
  };
  return aliases[key]||"";
}
function portailPrestationsUniques(d){
  var selected={}, customs=[];
  (d&&Array.isArray(d.prestations)?d.prestations:[]).forEach(function(x){
    var canon=portailCreationCanonique(x);
    if(canon) selected[normName(canon)]=canon;
    else {
      var libre=String(x||"").trim();
      var nk=normName(libre);
      if(libre && nk!=="autre" && nk!=="autre demande" && nk!=="autre creation ou demande") customs.push(libre);
    }
  });
  String(d&&d.autrePrestation||"").split(/\n/).map(function(x){return x.trim();}).filter(Boolean).forEach(function(x){customs.push(x);});
  var out=PORTAIL_CREATIONS_STANDARD.filter(function(label){return !!selected[normName(label)];});
  var seen={}; out.forEach(function(x){seen[normName(x)]=true;});
  customs.forEach(function(x){var k=normName(x);if(k&&!seen[k]){seen[k]=true;out.push(x);}});
  return out;
}
function applyPortalSelectionsToMariage(m,d){
  if(!m||!d) return false;
  var selections=portailPrestationsUniques(d);
  if(!selections.length) return false;
  var oldArticles=Array.isArray(m.articles)?m.articles:[];
  var oldExtras=Array.isArray(m.prestationsComplementaires)?m.prestationsComplementaires:[];
  var usedArticles={}, usedExtras={};
  var nextArticles=selections.map(function(label){
    var key=normName(label), art=null, extra=null, i;
    for(i=0;i<oldArticles.length;i++){
      if(usedArticles[i]) continue;
      if(normName(oldArticles[i].label||"")===key){ art=oldArticles[i]; usedArticles[i]=true; break; }
    }
    ...
    else if(extra&&num(extra.prix)>0) out.prix=num(extra.prix);
    return out;
  });
  // Les choix du formulaire sont limités à la liste standard MyBusiness et deviennent les créations principales.
  ...
  if(changed){
    ...
    m.historique.unshift({date:new Date().toISOString(),texte:"Créations synchronisées avec les choix du formulaire portail"});
...
function demandeMariageCategorie(d){
  ...
  // V7.7.1 : un statut "transformée" n'est valide que si la fiche mariage existe réellement.
  // Une ancienne transformation incomplète reste récupérable sans créer de doublon.
  if(st==="transformee" && linkedMariageForDemande(d)) return "transformees";
...
function demandeFleursPremiumText(d){
  d=d||{};
  var flowers=Array.isArray(d.fleursPremium)?d.fleursPremium:[];
  var quantities=(d.fleursPremiumQuantites&&typeof d.fleursPremiumQuantites==='object')?d.fleursPremiumQuantites:{};
  var rows=[];
  flowers.forEach(function(name){
    if(!name)return;
    var qty=Math.max(1,Math.round(Number(quantities[name])||1));
    if(name==='Autre'){
      var other=String(d.fleursPremiumAutre||'').trim();
      rows.push((other?'Autre : '+other:'Autre')+' × '+qty);
    }else rows.push(name+' × '+qty);
  });
  if(rows.length)return rows.join(', ');
  return String(d.fleursAimees||'').trim()||'Non renseignées';
}
...
function viewDemandeMariageDetail(d){
  ...
  return '<div class="row-actions"><button class="btn ghost" data-action="dem-back">← Retour aux demandes</button><a class="btn ghost" href="portail-mariage.html?dossier='+encodeURIComponent(d.id)+'" target="_blank">Voir le portail cliente</a></div>'+(d.clientModificationPending?'<div class="card" style="background:#fff3bf;border-color:#e0b84b;"><b>⚠️ Modifications clientes à consulter</b><div class="muted" style="margin-top:4px;">Dernière modification : '+esc(d.lastClientModificationAt?new Date(d.lastClientModificationAt).toLocaleString('fr-FR'):'date inconnue')+'</div>'+demandeLastChangesSummary(d)+'</div>':'')+'<div class="card"><div class="flexb"><div><h2 style="margin:0;">'+esc((d.prenom||"")+" "+(d.nom||""))+'</h2><p class="muted">Demande reçue le '+esc(receivedAt?receivedAt.slice(0,10):"Date non renseignée")+'</p></div><select id="demStatut">'+Object.keys(DEMANDE_STATUTS).map(function(k){return '<option value="'+k+'"'+((d.statut||"nouvelle")===k?' selected':'')+'>'+esc(DEMANDE_STATUTS[k])+'</option>';}).join('')+'</select></div><div class="form-grid"><label>Email<input id="demEmail"'+demandeFieldStyle(d,'email')+' value="'+esc(d.email||"")+'"></label><label>Téléphone<input id="demTel"'+demandeFieldStyle(d,'tel')+' value="'+esc(d.tel||"")+'"></label><label>Date du mariage<input id="demDate"'+demandeFieldStyle(d,'dateMariage')+' type="date" value="'+esc(d.dateMariage||"")+'"></label><label>Adresse postale<input id="demAdresse"'+demandeFieldStyle(d,'adressePostale')+' value="'+esc(d.adressePostale||"")+'"></label><label>Code postal<input id="demCodePostal"'+demandeFieldStyle(d,'codePostal')+' value="'+esc(d.codePostal||"")+'"></label><label>Lieu / ville<input id="demLieu"'+demandeFieldStyle(d,'ville')+' value="'+esc(d.lieu||d.ville||"")+'"></label><label>Thème / style<input id="demTheme"'+demandeFieldStyle(d,'style')+' value="'+esc(d.style||d.theme||"")+'"></label><label>Budget<input id="demBudget" value="'+esc(d.budget||"")+'"></label></div>'+demandePrestationsDetailHtml(d)+'<p><b>Couleurs :</b> '+esc(d.couleurs||"Non renseignées")+'</p><p><b>Fleurs premium stabilisées souhaitées :</b> '+esc(demandeFleursPremiumText(d))+'</p>'+(d.fleursRefusees?'<p><b>Éléments refusés :</b> '+esc(d.fleursRefusees)+'</p>':'')+((d.description||d.commentaire)?'<p><b>Ancienne description :</b><br>'+esc(d.description||d.commentaire)+'</p>':'')+'<p><b>Rendez-vous téléphonique souhaité :</b> '+esc(d.souhaiteRdvTelephonique==="oui"?"Oui":d.souhaiteRdvTelephonique==="non"?"Non":"Non renseigné")+'</p>'+(d.souhaiteRdvTelephonique==="oui"?'<p><b>Créneau demandé :</b> '+esc([d.rdvDateSouhaitee,d.rdvHeureSouhaitee].filter(Boolean).join(" à ")||"Non renseigné")+'</p>':'')+'<p class="muted"><b>Disponibilités :</b> du samedi au lundi de 9 h à 20 h ; du mardi au vendredi de 18 h à 20 h.</p><label>Notes internes<textarea id="demNotes" rows="4">'+esc(d.notesInternes||"")+'</textarea></label>'+(photos?'<h3>Inspirations</h3><div class="row-actions">'+photos+'</div>':'')+(demandeTransformationOrpheline(d)?'<div style="margin-top:14px;padding:11px 12px;border:1px solid #e0b84b;border-radius:10px;background:#fff8e8;"><b>⚠️ Transformation incomplète détectée</b><div class="muted" style="margin-top:4px;">La demande est marquée transformée mais aucune fiche mariage liée n’existe. Tu peux recréer la fiche ci-dessous.</div></div>':'')+'<div class="row-actions"><button class="btn primary" data-action="dem-save-'+d.id+'">Enregistrer</button>'+((d.statut!=="transformee"||demandeTransformationOrpheline(d))?'<button class="btn gold" data-action="dem-transform-'+d.id+'">'+(demandeTransformationOrpheline(d)?'🔧 Recréer la fiche mariage':'Créer la fiche mariage')+'</button>':'<span class="badge">Fiche mariage créée</span>')+'<button class="btn danger ghost" data-action="dem-delete-'+d.id+'">Supprimer</button></div></div>'+demandeChangeHistoryHtml(d);
}
function saveDemandeFromView(d){
  d.email=val("demEmail"); d.tel=val("demTel"); d.dateMariage=val("demDate"); d.adressePostale=val("demAdresse"); d.codePostal=val("demCodePostal"); d.lieu=val("demLieu"); d.theme=val("demTheme"); d.budget=val("demBudget"); d.notesInternes=val("demNotes"); d.statut=val("demStatut")||d.statut||"nouvelle"; d.updatedAt=new Date().toISOString();
}
function transformDemandeToMariage(d){
  var existing=linkedMariageForDemande(d);
  if(existing){
    d.mariageId=existing.id;
    d.statut="transformee";
    return existing;
  }
  // Ancienne transformation incomplète : l'identifiant orphelin ne doit pas bloquer la réparation.
  if(d.mariageId && !getMariage(d.mariageId)) d.mariageId="";
  var full=((d.prenom||"")+" "+(d.nom||"")).trim();
  var adresseComplete=[d.adressePostale||"",[d.codePostal||"",d.ville||d.lieu||""].filter(Boolean).join(" ")].filter(Boolean).join(", ");
  var c=(state.clients||[]).find(function(x){return (d.email&&x.email===d.email)||(d.tel&&x.tel===d.tel);});
  if(!c){ c={id:uid(),nom:full,adresse:adresseComplete,email:d.email||"",tel:d.tel||"",canal:d.canal||"Portail mariage",notes:"Demande créée depuis le portail mariage",createdAt:new Date().toISOString()}; state.clients.unshift(c); }
  else if(!c.adresse && adresseComplete) c.adresse=adresseComplete;
  var m={id:uid(),clientId:c.id,nom:full,email:d.email||"",tel:d.tel||"",canalCommunication:d.canal||"Portail mariage",dateMariage:d.dateMariage||"",dateLivraison:"",modeLivraison:"",adressePostale:d.adressePostale||"",codePostal:d.codePostal||"",lieu:d.lieu||d.ville||"",theme:[d.style,d.couleurs].filter(Boolean).join(" · "),budget:d.budget||"",besoins:demandePrestationsText(d),synthese:d.description||d.commentaire||"",fleursAimees:d.fleursAimees||"",fleursPremium:Array.isArray(d.fleursPremium)?d.fleursPremium.slice():[],fleursPremiumQuantites:(d.fleursPremiumQuantites&&typeof d.fleursPremiumQuantites==="object")?Object.assign({},d.fleursPremiumQuantites):{},fleursPremiumAutre:d.fleursPremiumAutre||"",statut:"contact",livre:false,dateLivree:"",relance:"",devisEnvoye:false,devisDate:"",factureEnvoyee:false,factureDate:"",devisLie:"",articles:portailPrestationsUniques(d).map(function(label){ var qty=Math.max(1,Number(d.prestationQuantites&&d.prestationQuantites[label])||1); var article={id:uid(),label:label,fait:false,quantite:qty}; if(d.rubansPersonnalises&&d.rubansPersonnalises[label]) article.textePersonnalisation=String(d.rubansPersonnalises[label]); return article; }),prestationsComplementaires:[],portalSelectionsSyncedV4:true,coutMatieres:"",todoMariage:[],medias:portalPhotosToMariageMedias(d.photos),historique:[{date:new Date().toISOString(),texte:"Fiche créée depuis la demande portail"}],createdAt:todayISO(),sourceDemandeId:d.id,ownerUid:d.ownerUid||"",souhaiteRdvTelephonique:d.souhaiteRdvTelephonique||"",rdvDateSouhaitee:d.rdvDateSouhaitee||"",rdvHeureSouhaitee:d.rdvHeureSouhaitee||""};
...
var DEFAULT_ARTICLES=PORTAIL_CREATIONS_STANDARD.slice();
var STATUT_MAR={
...
function viewMariagePrestationsComplementaires(m){
  ...
  var html='<div class="card" id="marExtraBox" style="background:var(--cream);"><div class="flexb"><div><h3 style="margin:0;">Prestations complémentaires</h3>'+ 
    '<p class="muted" style="margin:4px 0 0;"><b>Lecture seule :</b> toute modification d’une ligne commerciale doit être faite directement dans le devis lié.</p></div>'+ 
    '<span class="chip" id="marExtraTotal">Total compléments : '+euro(total)+'</span></div>';
  if(!lignes.length){
    html+='<p class="muted" style="margin:12px 0 0;">Aucune prestation complémentaire.</p>';
  }else{
    html+='<div class="scroll" style="margin-top:12px;"><table><thead><tr style="text-align:left;color:var(--ink-s);font-size:12px;"><th style="padding:6px;">Libellé</th><th style="padding:6px;">Catégorie</th><th style="padding:6px;width:82px;">Qté</th><th style="padding:6px;width:115px;">Prix unit.</th><th style="padding:6px;width:110px;text-align:right;">Total</th></tr></thead><tbody>';
    lignes.forEach(function(l){
      html+='<tr style="border-top:1px solid var(--line);">'+
        '<td style="padding:8px 6px;">'+esc(l.designation||'')+'</td>'+
        '<td style="padding:8px 6px;">'+esc(l.type==='service'?'Service':'Bien')+'</td>'+
        '<td style="padding:8px 6px;">'+esc(l.qte||1)+'</td>'+
        '<td style="padding:8px 6px;">'+euro(num(l.prix))+'</td>'+
        '<td style="padding:8px 6px;text-align:right;font-weight:700;white-space:nowrap;">'+euro(num(l.qte)*num(l.prix))+'</td>'+
      '</tr>';
    });
    html+='</tbody></table></div>';
  }
...
function mariageRdvDefault(){
  return {
    ...
    creationSelections:[], autreCreation:""
  };
}
...
function captureMariageRdvDraft(){
  ...
  ["nom","email","tel","canalCommunication","dateMariage","dateLivraison","modeLivraison","lieu","theme","couleurs","budget","inspirations","fleursAimees","fleursAEviter","styleBouquet","contraintes","notes","relance","autreCreation"].forEach(function(k){ var id="rdv"+k.charAt(0).toUpperCase()+k.slice(1); d[k]=rdvVal(id); });
  d.creationSelections=Array.prototype.slice.call(document.querySelectorAll('[data-rdv-creation]:checked')).map(function(el){return el.value;});
  return d;
}
function mariageRdvArticles(d){
  var labels=[], seen={};
  (Array.isArray(d.creationSelections)?d.creationSelections:[]).forEach(function(label){
    var canon=portailCreationCanonique(label)||String(label||"").trim();
    var k=normName(canon); if(canon&&k&&!seen[k]){seen[k]=true;labels.push(canon);}
  });
  String(d.autreCreation||"").split(/\n/).map(function(x){return x.trim();}).filter(Boolean).forEach(function(label){
    var k=normName(label); if(k&&!seen[k]){seen[k]=true;labels.push(label);}
  });
  return labels.map(function(label){return {id:uid(),label:label,fait:false};});
}
function mariageRdvSynthese(d){
  var articles=mariageRdvArticles(d).map(function(a){return "- "+a.label;}).join("\n")||"Aucune création sélectionnée";
  var txt=[];
...
function rdvCreationCheck(label,selected){ var checked=(selected||[]).some(function(x){return normName(x)===normName(label);}); return '<label class="checkrow" style="border-bottom:none;cursor:pointer;align-items:flex-start;"><input type="checkbox" data-rdv-creation value="'+esc(label)+'"'+(checked?' checked':'')+'><div style="flex:1;">'+esc(label)+'</div></label>'; }
function mariageRdvMediaHtml(d){
  var medias=(d.medias||[]).map(function(md){
    if(md.type==="image") return '<div style="position:relative;display:inline-block;"><img class="thumb" src="'+(md.dataUrl||md.data||"")+'" data-action="mar-rdv-media-open-'+md.id+'" title="Ouvrir '+esc(md.name||"image")+'"><button class="btn small danger" data-action="mar-rdv-media-del-'+md.id+'" style="position:absolute;top:-6px;right:-6px;padding:2px 7px;border-radius:50%;background:#fff;">×</button></div>';
...
function viewMariageRdvWizard(){
  ...
  '<div class="card"><h3 style="margin:0 0 10px;">4. Créations souhaitées</h3><p class="muted" style="margin-top:0;">Sélectionne uniquement les créations demandées par la future mariée.</p><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:6px;">'+PORTAIL_CREATIONS_STANDARD.map(function(label){return rdvCreationCheck(label,d.creationSelections||[]);}).join("")+'</div><label class="field" style="margin-top:12px;"><span>Autre création ou demande</span><input id="rdvAutreCreation" value="'+esc(d.autreCreation||"")+'" placeholder="Ex : décoration particulière, fleurs pour urne…"></label></div>'+ 
...
function mariageAtelierReferenceDocument(m){
  ...
  if(d) return {kind:"devis",doc:d,label:(d.numero||"Devis")};
  var facs=mariageFacturesLiees(m).slice().sort(function(a,b){return String(b.date||b.createdAt||"").localeCompare(String(a.date||a.createdAt||""));});
  if(facs.length) return {kind:"facture",doc:facs[0],label:(facs[0].numero||"Facture")};
  return null;
}
function mariageAtelierProductionRows(m){
  var rows=[];
  (m&&m.articles||[]).forEach(function(a){
    if(!a||!String(a.label||"").trim()) return;
    rows.push({id:a.id||"",label:String(a.label||"Création"),qty:Math.max(1,Number(a.quantite)||1),done:!!a.fait,custom:String(a.textePersonnalisation||""),atelierNote:String(a.noteAtelier||"")});
  });
  // Si une ancienne fiche ne contient pas encore d'articles structurés, on utilise le devis lié en lecture seule.
  if(!rows.length){
    var d=mariageDevisLie(m);
    (d&&d.lignes||[]).forEach(function(l,i){
      if(!l||!String(l.designation||"").trim()) return;
      rows.push({id:"",label:String(l.designation),qty:Math.max(1,Number(l.qte)||1),done:false,custom:"",atelierNote:"",readonly:true});
    });
  }
  return rows;
}
function mariageAtelierMediasHTML(m){
  var imgs=(m&&m.medias||[]).filter(function(md){return md&&md.type==="image"&&(md.dataUrl||md.data||md.url);});
  if(!imgs.length) return '<div class="atelier-empty">Aucune photo d’inspiration enregistrée.</div>';
  return '<div class="atelier-photo-grid">'+imgs.map(function(md){
    var src=md.dataUrl||md.data||md.url||"";
    return '<button class="atelier-photo" data-action="mar-media-open-'+esc(md.id)+'" title="Ouvrir la photo"><img src="'+esc(src)+'" alt="Inspiration"></button>';
  }).join('')+'</div>';
}
function mariageAtelierCustomTexts(m){
  return (m&&m.articles||[]).filter(function(a){return a&&String(a.textePersonnalisation||"").trim();});
}
function mariageAtelierPremiumText(m){
  var vals=[];
  if(m&&m.fleursPremium&&Array.isArray(m.fleursPremium)) vals=vals.concat(m.fleursPremium);
  if(m&&m.premiumFlowers&&Array.isArray(m.premiumFlowers)) vals=vals.concat(m.premiumFlowers);
  if(m&&m.fleursAimees) vals.push(m.fleursAimees);
  vals=vals.map(function(x){return String(x||"").trim();}).filter(Boolean);
  return vals.filter(function(v,i,a){return a.indexOf(v)===i;}).join(" · ");
}
function viewMariageAtelier(m){
  var rows=mariageAtelierProductionRows(m);
  var ref=mariageAtelierReferenceDocument(m);
  var custom=mariageAtelierCustomTexts(m);
  var premium=mariageAtelierPremiumText(m);
  var facs=mariageFacturesLiees(m);
  var d=mariageDevisLie(m);
  var done=rows.filter(function(r){return r.done;}).length;
  var pct=rows.length?Math.round((done/rows.length)*100):0;
  var prod=rows.length?rows.map(function(r){
    var checkbox=r.readonly?'<span class="atelier-readonly-dot">•</span>':'<input class="atelier-check" type="checkbox" data-action="mar-art-toggle-'+esc(r.id)+'"'+(r.done?' checked':'')+'>';
    return '<div class="atelier-production-row '+(r.done?'is-done':'')+'">'+checkbox+'<div class="atelier-production-label"><b>'+esc(r.label)+'</b>'+(r.custom?'<div class="atelier-custom-inline">« '+esc(r.custom)+' »</div>':'')+(r.readonly?'':'<textarea class="atelier-note-input" data-mar-atelier-note="'+esc(r.id)+'" placeholder="Note atelier interne : teinte, finition, détail de montage…">'+esc(r.atelierNote||'')+'</textarea>')+'</div><span class="atelier-qty">× '+esc(String(r.qty))+'</span></div>';
  }).join(''):'<div class="atelier-empty">Aucune création renseignée dans cette fiche.</div>';
  var docs='';
  if(d) docs+='<button class="btn soft" data-action="notif-open-devis-'+esc(d.id)+'">📄 '+esc(d.numero||"Ouvrir le devis")+'</button>';
  facs.forEach(function(f){ docs+='<button class="btn soft" data-action="notif-open-facture-'+esc(f.id)+'">💰 '+esc(f.numero||"Ouvrir la facture")+'</button>'; });
  if(!docs) docs='<span class="atelier-empty">Aucun devis ou facture lié.</span>';
  var contact=[m.tel||"",m.email||""].filter(Boolean).map(esc).join(' · ');
  return '<style>'+ 
    '.atelier-overlay{position:fixed;inset:0;z-index:2000;background:#fffaf7;overflow:auto;color:var(--ink);padding:18px 22px 42px;}'+
    '.atelier-shell{max-width:1500px;margin:0 auto;}'+
    '.atelier-toolbar{position:sticky;top:0;z-index:5;background:rgba(255,250,247,.96);backdrop-filter:blur(8px);padding:8px 0 12px;border-bottom:1px solid var(--line);display:flex;justify-content:space-between;gap:12px;align-items:center;}'+
    '.atelier-title{font-size:28px;color:var(--bordeaux);font-weight:800}.atelier-sub{color:var(--ink-s);font-size:13px;margin-top:2px;}'+
    '.atelier-head{display:grid;grid-template-columns:1.35fr .65fr;gap:14px;margin:16px 0;}'+
    '.atelier-card{background:white;border:1px solid var(--line);border-radius:16px;padding:16px;box-shadow:0 3px 12px rgba(80,50,50,.04);}'+
    '.atelier-card h2,.atelier-card h3{margin:0 0 10px;color:var(--bordeaux);}'+
    '.atelier-meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px 18px;font-size:14px}.atelier-meta b{display:block;font-size:12px;color:var(--ink-s);margin-bottom:2px;}'+
    '.atelier-progress{height:9px;border-radius:999px;background:#eee5df;overflow:hidden;margin-top:12px}.atelier-progress>span{display:block;height:100%;background:var(--blush);width:var(--pct);}'+
    '.atelier-main{display:grid;grid-template-columns:minmax(420px,1.15fr) minmax(360px,.85fr);gap:14px;align-items:start;}'+
    '.atelier-production-row{display:grid;grid-template-columns:34px 1fr auto;gap:10px;align-items:start;border-bottom:1px solid var(--line);padding:10px 2px}.atelier-production-row:last-child{border-bottom:0}.atelier-production-row.is-done{opacity:.55}.atelier-production-row.is-done .atelier-production-label>b{text-decoration:line-through}.atelier-check{width:22px;height:22px;accent-color:var(--bordeaux);margin-top:2px}.atelier-readonly-dot{font-size:24px;text-align:center;color:var(--bordeaux)}.atelier-production-label{font-size:15px}.atelier-qty{font-size:16px;font-weight:800;color:var(--bordeaux);white-space:nowrap}.atelier-custom-inline{font-size:12px;color:var(--ink-s);margin-top:3px;font-style:italic}.atelier-note-input{width:100%;min-height:58px;margin-top:7px;padding:8px 10px;border:1px solid var(--line);border-radius:9px;background:#fffdfb;font:inherit;font-size:13px;line-height:1.35;resize:vertical}.atelier-note-input:focus{outline:2px solid var(--blush-s);border-color:var(--blush)}'+
    '.atelier-photo-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.atelier-photo{border:0;background:transparent;padding:0;cursor:pointer}.atelier-photo img{width:100%;height:180px;object-fit:cover;border-radius:12px;border:1px solid var(--line);display:block}'+
    '.atelier-text{white-space:pre-wrap;line-height:1.5;background:#fffdfb;border:1px solid var(--line);border-radius:12px;padding:12px;min-height:72px}.atelier-empty{color:var(--ink-s);font-style:italic}.atelier-docs{display:flex;flex-wrap:wrap;gap:8px}.atelier-section-gap{margin-top:14px}.atelier-reference{font-size:12px;color:var(--ink-s);margin-bottom:8px}'+
  ...
  '</style><div class="atelier-overlay"><div class="atelier-shell">'+
    ...
    '</div><div>'+ 
      ...
      (String(m.synthese||'').trim()?'<section class="atelier-card atelier-section-gap"><h2>💐 Synthèse du projet</h2><div class="atelier-text">'+esc(m.synthese)+'</div></section>':'')+
      (custom.length?'<section class="atelier-card atelier-section-gap"><h2>🎀 Personnalisations</h2>'+custom.map(function(a){return '<div style="padding:7px 0;border-bottom:1px solid var(--line);"><b>'+esc(a.label||'Création')+'</b><div>« '+esc(a.textePersonnalisation)+' »</div></div>';}).join('')+'</section>':'')+
      (premium?'<section class="atelier-card atelier-section-gap"><h2>🌸 Fleurs / éléments premium</h2><div class="atelier-text">'+esc(premium)+'</div></section>':'')+
      '<section class="atelier-card atelier-section-gap"><h2>📁 Documents</h2><div class="atelier-docs">'+docs+'</div></section>'+ 
    '</div></div>'+ 
  '</div></div>';
}
...
function viewMariageDetail(m){
  ...
  var infos='<div class="card"><div class="flexb" style="margin-bottom:6px;"><h3 style="margin:0;">Fiche contact & projet</h3>'+
    ...
    '<label class="field"><span>🌸 Fleurs premium stabilisées souhaitées</span><textarea id="marFleursAimees" style="min-height:72px;" placeholder="Ex : Rose × 3, Pivoine × 1">'+esc(m.fleursAimees||"")+'</textarea><div class="hint">Repris automatiquement de la demande cliente au moment de la création de la fiche mariage. Les quantités sont conservées. Cette information n’altère aucun devis existant.</div></label>'+
...
function clientEmailOrPrompt(kind, doc){
  ...
  // Si le document appartient à un mariage, l'adresse confirmée devient aussi celle du dossier et de la fiche cliente.
  var linked=findLinkedMariageForDoc(kind,doc);
  if(linked){ linked.email=email; syncMariageContactToClient(linked); }
  return email;
}
...
async function testDownloadDocPdf(){
  ...
  if(!prepareDocumentContactAndAmount(ui.preview.kind,ui.preview.doc)) return;
  saveCache();
  try{
...
function findLinkedMariageForDoc(kind, doc){
  if(!doc) return null;
  var devisId=kind==="devis"?(doc.id||""):(doc.devisId||doc.devisLie||"");
  if(!devisId) return null;
  return (state.mariages||[]).find(function(m){return m.devisLie===devisId;})||null;
}
function recoverDevisLinesIfZero(d){
  if(!d || totals(d.lignes||[],state.settings.partService).total>0) return false;
  var sources=[];
  (state.factures||[]).forEach(function(f){
    if(f.devisId===d.id && totals(f.lignes||[],state.settings.partService).total>0) sources.push({kind:"facture",date:f.datePaiement||f.date||"",lignes:f.lignes});
  });
  (state.commandes||[]).forEach(function(c){
    if(c.devisId===d.id && totals(c.lignes||[],state.settings.partService).total>0) sources.push({kind:"commande",date:c.dateCommande||c.createdAt||"",lignes:c.lignes});
  });
  sources.sort(function(a,b){return String(b.date||"").localeCompare(String(a.date||""));});
  if(!sources.length) return false;
  d.lignes=(sources[0].lignes||[]).map(function(l){return Object.assign({},l);});
  d.recoveredAt=new Date().toISOString();
  d.recoverySource=sources[0].kind;
  return totals(d.lignes,state.settings.partService).total>0;
}
function prepareDocumentContactAndAmount(kind, doc){
  var linked=findLinkedMariageForDoc(kind,doc);
  if(linked){
    if(ui&&ui.mariageOpen===linked.id) captureMariageInputs();
    syncMariageContactToClient(linked);
    if(kind==="devis") syncMariageLinkedDevis(linked,{silent:true,updateClient:false,syncLines:false});
    else if(doc.client) doc.client=Object.assign({},doc.client,devisClientFromMariage(linked));
  }
  if(kind==="devis" && totals(doc.lignes||[],state.settings.partService).total===0){
    if(recoverDevisLinesIfZero(doc)){
      toast("Montant du devis restauré depuis un document lié avant envoi.");
    }else{
      toast("Envoi bloqué : ce devis est à 0 €. Ouvre-le et vérifie ses lignes avant de le renvoyer.");
      return false;
    }
  }
  return true;
...
async function envoyerDocumentEmail(kind, doc){
  ...
  saveCache();
  var email=clientEmailOrPrompt(kind,doc); if(!email) return;
...
async function handleAction(action){
  ...
  if(action.indexOf("stock-sub-")===0){ ui.stockSub=action.slice(10); render(); window.scrollTo(0,0); return; }
  if(action.indexOf("atelier-library-edit-")===0){ ui.stockRecipeModel=action.slice(21); ui.atelierLibraryEditId=ui.stockRecipeModel; render(); return; }
  if(action.indexOf("atelier-library-recipe-")===0){ ui.stockRecipeModel=action.slice(23); ui.stockSub="recettes"; render(); window.scrollTo(0,0); return; }
  if(action.indexOf("atelier-library-duplicate-")===0){
    ui.stockRecipeModel=action.slice(26); action="atelier-library-duplicate";
  }
  if(action.indexOf("atelier-library-archive-")===0){
    ui.stockRecipeModel=action.slice(24); action="atelier-library-archive";
  }
  if(action==="atelier-library-new"){
    var nid="atelier_"+uid(); ui.atelierLibraryEditId=nid; ui.stockRecipeModel=nid; render(); return;
  }
  ...
  if(action==="site-cancel"){ ui.siteSaleDraft=null; ui.siteSaleEditingId=null; render(); return; }
  if(action.indexOf("site-edit-")===0){ editSiteSale(action.slice(10)); return; }
  ...
  if(action.indexOf("site-del-")===0){
    ...
    if(ui.siteSaleEditingId===sid){ ui.siteSaleEditingId=null; ui.siteSaleDraft=null; }
    ui.confirmDelete=null; saveCache(); render(); toast(saleToDelete&&isSiteSaleAtelierPayment(saleToDelete)?"Paiement atelier supprimé.":"Vente site supprimée."); return;
  }
  ...
  if(action.indexOf("mar-extra-add-")===0 || action.indexOf("mar-extra-del-")===0){ toast("Modification bloquée : modifie les lignes directement dans le devis lié."); return; }
  ...
  if(action==="mar-art-add" || action.indexOf("mar-art-del-")===0){ toast("Modification bloquée : modifie les lignes directement dans le devis lié."); return; }
...
function syncMariageContactToClient(m){
  ...
  if(!c){
    if(!String(m.nom||m.email||m.tel||"").trim()) return null;
    c={id:uid(),nom:m.nom||"",adresse:m.adresse||m.adresseCliente||"",email:m.email||"",tel:m.tel||"",canal:m.canalCommunication||"",anniversaire:"",notes:"",createdAt:todayISO(),updatedAt:new Date().toISOString()};
    state.clients.push(c);
    m.clientId=c.id;
    return c;
  }
  // Une modification volontaire dans la fiche mariage devient la nouvelle référence de contact.
  c.nom=m.nom||c.nom||"";
  c.email=m.email==null?(c.email||""):String(m.email);
  c.tel=m.tel==null?(c.tel||""):String(m.tel);
  c.canal=m.canalCommunication||c.canal||"";
...
function mariageLineKey(line){
  return normName((line&&line.designation)||"");
}
function mariageLinesForDevis(m){
  ...
  mariagePrestations(m).forEach(function(l){
    ...
    lignes.push({id:l.devisLineId, designation:l.designation||"Prestation complémentaire", type:l.type==="service"?"service":"bien", urssafType:l.type==="service"?"service":"bien", qte:l.qte||1, prix:r2(l.prix), mariageSource:"prestation"});
  });
  return lignes;
}
function mergeMariageLinesWithDevis(m, d){
  var existing=(d.lignes||[]).map(function(l){return Object.assign({},l);});
  var used={};
  return mariageLinesForDevis(m).map(function(next){
    var match=null;
    for(var i=0;i<existing.length;i++){
      if(used[i]) continue;
      if(existing[i].id===next.id || (mariageLineKey(existing[i]) && mariageLineKey(existing[i])===mariageLineKey(next))){ match=existing[i]; used[i]=true; break; }
    }
...
function syncMariageCreationsFromDevis(m,d){
  ...
  var oldExtras=mariagePrestations(m).slice();
  var usedArticles={}, usedExtras={};

  function sameId(a,l){
    var aid=String((a&&a.devisLineId)||""), lid=String((l&&l.id)||"");
    return !!(aid&&lid&&aid===lid);
  }
  function findArticle(l){
    var i;
    for(i=0;i<oldArticles.length;i++){ if(!usedArticles[i]&&sameId(oldArticles[i],l)){ usedArticles[i]=true; return oldArticles[i]; } }
    var key=mariageLineKey(l);
    for(i=0;i<oldArticles.length;i++){ if(!usedArticles[i]&&key&&normName(oldArticles[i].label||"")===key){ usedArticles[i]=true; return oldArticles[i]; } }
    return null;
  }
  function findExtra(l){
    var i;
    for(i=0;i<oldExtras.length;i++){ if(!usedExtras[i]&&sameId(oldExtras[i],l)){ usedExtras[i]=true; return oldExtras[i]; } }
    var key=mariageLineKey(l);
    for(i=0;i<oldExtras.length;i++){ if(!usedExtras[i]&&key&&normName(oldExtras[i].designation||"")===key){ usedExtras[i]=true; return oldExtras[i]; } }
    return null;
  }
  ...
  (d.lignes||[]).forEach(function(l){
    if(!l||!String(l.designation||"").trim()) return;
    var isService=(l.type==="service"||l.urssafType==="service");
    if(isService){
      var prevE=findExtra(l)||{};
      extras.push(Object.assign({},prevE,{
        id:prevE.id||uid(), devisLineId:l.id||prevE.devisLineId||uid(),
        designation:String(l.designation), type:"service", urssafType:"service",
        qte:Math.max(1,Number(l.qte)||1), prix:r2(l.prix)
      }));
    } else {
      var prevA=findArticle(l)||{};
      articles.push(Object.assign({},prevA,{
        id:prevA.id||uid(), devisLineId:l.id||prevA.devisLineId||uid(),
        label:String(l.designation), quantite:Math.max(1,Number(l.qte)||1),
        prix:r2(l.prix), fait:!!prevA.fait
      }));
    }
  });
  m.articles=articles;
...
document.addEventListener("input", function(e){
  ...
  if(t.hasAttribute&&t.hasAttribute("data-linefield")){
    ...
    scheduleMariageAutoSync({syncLines:true});
  } else if(t.hasAttribute&&t.hasAttribute("data-mar-atelier-note")){
    var mn=getMariage(ui.mariageOpen);
    var mnid=t.getAttribute("data-mar-atelier-note");
    var mna=mn&&(mn.articles||[]).find(function(x){return x.id===mnid;});
    if(mna){ mna.noteAtelier=t.value; mna.noteAtelierUpdatedAt=new Date().toISOString(); saveCache(); }
    return;
...
document.addEventListener("change", function(e){
  ...
  if(t.hasAttribute&&t.hasAttribute("data-marextra-type")){ refreshMariageExtraTotalsFromDOM(); scheduleMariageAutoSync({syncLines:true}); return; }
  ...
  if(t.getAttribute&&t.getAttribute("data-action")==="site-activite-change"){
    if(ui.siteSaleDraft){
      captureSiteSaleDraft(); ui.siteSaleDraft.activite=t.value;
      if(t.value==="Atelier"){
        ui.siteSaleDraft.nature="atelier_payment"; ui.siteSaleDraft.caContractuelInclus=true;
        setSiteDraftAtelier(ui.siteSaleDraft,ui.siteSaleDraft.atelierId,true);
      }else{
        ui.siteSaleDraft.nature="independent_sale"; ui.siteSaleDraft.caContractuelInclus=false;
        ui.siteSaleDraft.atelierId=""; ui.siteSaleDraft.atelierLabel=""; ui.siteSaleDraft.atelierParticipantId="";
      }
      render();
    }
    return;
  }
  if(t.getAttribute&&t.getAttribute("data-action")==="site-atelier-change"){
    if(ui.siteSaleDraft){ captureSiteSaleDraft(); setSiteDraftAtelier(ui.siteSaleDraft,t.value,true); render(); }
    return;
  }
  ...
  if(act==="mar-link"){ var ml=getMariage(ui.mariageOpen); if(ml){ captureMariageInputs(); ml.devisLie=t.value; var linkedNow=ml.devisLie?findDevis(ml.devisLie):null; if(linkedNow) syncMariageCreationsFromDevis(ml,linkedNow); saveCache(); render(); } return; }
